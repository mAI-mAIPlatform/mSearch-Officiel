...
const newTabPage = {
  ...
  personalizationRandomButton: document.getElementById('ntp-personalization-random'),
  personalizationClearButton: document.getElementById('ntp-personalization-clear'),
  personalizationResetButton: document.getElementById('ntp-personalization-reset'),
  blobInstance: null,
  reminders: [],
  clockTimer: null,
  ...
  applyDisplayPreferences: function () {
    const density = localStorage.getItem(DENSITY_STORAGE_KEY) === 'compact' ? 'compact' : 'comfortable'
    const glass = ['balanced', 'soft', 'strong'].includes(localStorage.getItem(GLASS_STORAGE_KEY))
      ? localStorage.getItem(GLASS_STORAGE_KEY)
      : 'balanced'
  ...
  bindPersonalizationControls: function () {
    ...
    if (newTabPage.personalizationRandomButton) {
      newTabPage.personalizationRandomButton.addEventListener('click', function () {
        newTabPage.applyRandomBackground()
      })
    }

    if (newTabPage.personalizationClearButton) {
      newTabPage.personalizationClearButton.addEventListener('click', async function () {
        try {
    ...
    if (newTabPage.personalizationResetButton) {
      newTabPage.personalizationResetButton.addEventListener('click', function () {
        newTabPage.resetPersonalizationSettings()
      })
    }
  },
